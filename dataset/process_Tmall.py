...
for s in list(sess_clicks):
    ...
    filseq = list(filter(lambda i: iid_counts[i] >= 5, curseq))  # 只保留出现次数>=5的item
    if len(filseq) < 2 or len(filseq) > 40:  # 过长或过短都过滤掉
        del sess_clicks[s]
        del sess_date[s]
    else:
...
sorted_date = sorted(sess_date.items(), key=operator.itemgetter(1), reverse=True)
# for _, date in dates:
#     if maxdate < date:
#         maxdate = date
maxdate = sorted_date[0][1]
print("最大时间戳:  {}".format(maxdate))
...
tra = (tr_seqs, tr_labs)  # sequence  label
tes = (te_seqs, te_labs)
print('train_test')
print(len(tr_seqs))
print(len(te_seqs))
...
if not os.path.exists('tmall'):
    os.makedirs('tmall')
pickle.dump(tra, open('tmall/train.pkl', 'wb'))
pickle.dump(tes, open('tmall/test.pkl', 'wb'))
pickle.dump(tra_seqs, open('tmall/all_train_seq.pkl', 'wb'))
